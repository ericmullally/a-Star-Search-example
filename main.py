    ...
    def makeGrid(self, len: int, width: int) -> list:
        ...
        for i in range(0, len):
            ...
            for j in range(0, width):  
                ...
                # newButton.clicked.connect(self.buttonClick)
    ...
    def setNeighbors(self):
        ...
        for i in range(0, len(self.nodes)):
            for j in range(0, len(self.nodes[i]) ):
                ...
                rAbove = self.nodes[i-1][j + 1] if i != 0 and j != 15 else None 
                lMid = self.nodes[i][j-1] if j != 0 else None
                rMid = self.nodes[i][j+1] if j != 15 else None
                lBottom = self.nodes[i+1][j-1] if i != 15 and j != 0 else None
                dBotttom = self.nodes[i+1][j] if i != 15 else None 
                rBottom = self.nodes[i+1][j+1] if i != 15 and j != 15 else None
